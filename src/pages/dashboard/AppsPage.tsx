...
import {
  ...
  Crown,
  Edit3,
  Loader2 // Importante: Import do Loader
} from 'lucide-react';
...
const AppsPage: React.FC = () => {
  ...
  // Pegamos o loading aqui para proteger a renderização
  ...
  // 🛡️ TRAVA DE SEGURANÇA (O Fim do "Pisca-Pisca")
  // Enquanto o AuthContext estiver pensando, mostramos um spinner.
  // Isso impede que o código abaixo rode com profile = null e assuma Starter errado.
  if (loading) {
    return (
  ...
  // --- LÓGICA DE PLANOS ---
  // Se o perfil ainda não carregou (raro por causa da trava acima), assume starter.
  // Removemos 'free' e 'trial' da lógica padrão.
    ...
    <div className="space-y-8 font-['Outfit'] pb-20">
      ...
      {isLimitReached && (
        <div className="bg-orange-50 dark:bg-orange-900/10 border border-orange-100 dark:border-orange-900/30 rounded-2xl p-6 flex flex-col md:flex-row items-center justify-between gap-6 animate-slide-up shadow-sm">
          ...
          <Link
            to="/dashboard/plans"
            className="flex items-center justify-center gap-2 px-8 py-4 bg-brand-coral hover:bg-orange-600 text-white rounded-xl font-black uppercase tracking-widest text-[10px] transition-all shadow-lg shadow-orange-500/20 hover:shadow-orange-500/30 whitespace-nowrap w-full md:w-auto transform hover:-translate-y-0.5"
          >
            Fazer Upgrade Agora
      ...
      {apps.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 md:gap-8">
          {apps.map((app, index) => (
            <div
              key={app.id}
              className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 overflow-hidden hover:shadow-xl hover:border-brand-blue/30 dark:hover:border-brand-blue/30 transition-all duration-500 group animate-slide-up"
            ...
            >
              ...
              <div className="p-6 md:p-8 pb-6 flex flex-col items-center">
                <div className="relative mb-6">
                  {app.logo ? (
                    ...
                    <div
                      className="w-24 h-24 rounded-2xl flex items-center justify-center shadow-xl group-hover:scale-110 transition-transform duration-500 border-4 border-white dark:border-slate-700"
                ...
                <div className="flex items-center gap-3 bg-slate-50/80 dark:bg-slate-900/50 border border-slate-100 dark:border-slate-700 rounded-xl p-3 group/link">
                  ...
                  <div className="flex items-center gap-1">
                    <button
                      ...
                      className="p-2 text-slate-400 hover:text-brand-blue hover:bg-white dark:hover:bg-slate-800 hover:shadow-md rounded-lg transition-all"
                      title="Copiar link"
                    >
                      {copiedId === app.id ? (
              ...
              <div className="px-6 md:px-8 pb-8 space-y-3">
                ...
                <button
                  ...
                  className="flex items-center justify-center gap-3 w-full py-3.5 bg-red-50 dark:bg-red-900/10 hover:bg-red-100 dark:hover:bg-red-900/20 text-red-500 dark:text-red-400 rounded-xl font-bold transition-all border border-transparent hover:border-red-200 dark:hover:border-red-800 text-sm"
                >
          ...
          <div className="relative bg-white dark:bg-slate-800 rounded-2xl shadow-2xl max-w-md w-full p-6 md:p-10 animate-slide-up overflow-hidden">
            ...
            <div className="flex flex-col gap-3">
              <button
                ...
                className="w-full py-4 bg-red-500 hover:bg-red-600 text-white rounded-xl font-black uppercase tracking-widest text-xs shadow-xl shadow-red-500/20 transition-all active:scale-95"
              >
                Sim, apagar permanentemente
              </button>
              <button
                onClick={() => setDeleteModalOpen(false)}
                className="w-full py-4 bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 text-slate-600 dark:text-slate-300 rounded-xl font-black uppercase tracking-widest text-xs transition-all"
              >
                Cancelar e Manter
              </button>
            </div>
