...
import {
  ...
  ChevronUp, Video, Loader2, FileText
} from 'lucide-react';
import { supabase } from '../../lib/supabase';
import Button from '../../components/Button';
import CreateProductModal from '../../components/modals/CreateProductModal'; // Certifique-se que o caminho está certo
...
const ProductsPage: React.FC = () => {
  ...
  // ✅ Função Auxiliar para detectar se é ID ou Slug
  ...
  const fetchProducts = async () => {
    try {
      ...
      // 1. Busca Inteligente do App (Resolve o problema do produto não aparecer)
      let appId = null;
      let query = supabase.from('apps').select('id');

      if (appSlug && isUUID(appSlug)) {
        query = query.eq('id', appSlug); // Se a URL tem ID, busca por ID
      } else if (appSlug) {
        query = query.eq('slug', appSlug); // Se tem nome, busca por nome
      }
      ...
      // 2. Busca produtos vinculados a esse App
  ...
  return (
    <div className="space-y-8 font-sans pb-32 animate-fade-in max-w-6xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 border-b border-slate-200 dark:border-slate-800 pb-6">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 text-base mt-2 leading-relaxed max-w-2xl">
            Gerencie o núcleo do seu InfoApp: Cursos, Mentorias e Ebooks.
          </p>
        </div>
      ...
      <div className="space-y-6">
        {products.length === 0 ? (
          // ✅ CORREÇÃO VISUAL: Retângulo Central com bordas ajustadas (rounded-2xl)
          <div className="text-center py-24 bg-white dark:bg-slate-900 rounded-2xl border-2 border-dashed border-slate-200 dark:border-slate-800 shadow-sm animate-scale-in">
            ...
            <p className="text-slate-500 dark:text-slate-400 font-medium mb-8 max-w-md mx-auto">
              Seu app está vazio. Crie seu produto principal para começar a adicionar conteúdo.
            </p>
          ...
          products.map((product) => {
            ...
            return (
              <div key={product.id} className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden shadow-sm hover:shadow-md transition-all group">
                ...
                {/* Área de Conteúdo Expandida (Estilo Husky) */}
                {isExpanded && (
                  <div className="bg-slate-50 dark:bg-slate-950/50 border-t border-slate-200 dark:border-slate-800 p-8 animate-slide-up">
                    <div className="flex justify-between items-center mb-6">
                      <h4 className="text-sm font-bold text-slate-900 dark:text-white">Conteúdo do Produto</h4>
                    ...
                    <div className="space-y-4">
                      {product.modules && product.modules.length > 0 ? (
                        ...
                        // Estado Vazio Interno do Produto
                        <div className="text-center py-10 border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-2xl bg-white dark:bg-slate-900">
      ...
      {/* Modal de Criação de Produto (Integrado e Funcionando) */}
      <CreateProductModal
        isOpen={isProductModalOpen}
        onClose={() => setIsProductModalOpen(false)}
        onSuccess={() => {
          fetchProducts(); // Isso recarrega a lista assim que criar!
