...
type SupportType = 'email' | 'whatsapp' | null;
...
const AppBuilder: React.FC = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    support_type: null as SupportType,
    support_value: ''
  });
    ...
    if (editMode && appIdToEdit) {
      ...
      if (app) {
        setFormData({
          ...
          support_type: app.support_type || null,
          support_value: app.support_value || ''
        });
      }
    }
  ...
  const updateBanner = (index: number, field: 'image_url' | 'link', value: string) => {
    ...
    <div className="flex h-[calc(100vh-100px)] overflow-hidden bg-white dark:bg-slate-950 font-['inter'] rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm animate-fade-in">
      ...
      <div className="w-full lg:w-[500px] xl:w-[600px] flex flex-col bg-white dark:bg-slate-950 border-r border-slate-200 dark:border-slate-800 relative z-10">
        ...
        <div className="h-16 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between px-6 bg-white dark:bg-slate-950">
          <div className="flex items-center gap-3">
            <button
              ...
              className="p-2 -ml-2 text-slate-400 hover:text-slate-600 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg transition-colors"
            >
        ...
        <div className="flex-1 overflow-y-auto p-6 md:p-8 relative">
          ...
          {isLimitReached && (
            <div className="absolute inset-0 z-20 bg-white/80 dark:bg-slate-950/80 backdrop-blur-sm flex flex-col items-center justify-center p-8 text-center">
              ...
              <Button
                ...
                className="w-full max-w-xs text-xs font-bold uppercase"
              >
                Fazer Upgrade
              </Button>
            </div>
            ...
            <div className="space-y-6 animate-slide-up">
              ...
              <div>
                ...
                <p className="text-[10px] text-slate-400 mt-2 font-medium">
                  Use apenas letras minúsculas e hifens. Ex: ingles-do-joao
                </p>
              </div>
            ...
            <div className="space-y-8 animate-slide-up">
              <div className="flex items-center gap-3 mb-2">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500">Personalize as cores, logo e banners.</p>
                </div>
              </div>
                  ...
                  {isUploading ? (
                    ...
                    <div className="relative z-10 text-center">
                      <img
                        src={formData.logo}
                        alt="Logo"
                        className="h-20 object-contain mx-auto mb-4 drop-shadow-sm"
                      />
                      <button
                    ...
                    >
                      <div className="flex items-center gap-2 mb-3">
                        ...
                        <span className="text-xs font-bold text-slate-600 dark:text-slate-300">
                          Banner {index + 1} {index === 0 && '(Principal)'}
                        </span>
                      </div>
                        ...
                        {isUploading && !banner.image_url ? (
                          ...
                          <>
                            ...
                            <p className="text-xs text-slate-500">
                              {index === 0 ? 'Clique para adicionar imagem' : 'Opcional'}
                            </p>
                          </>
                        )}
                      </div>
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <input
                            ...
                            className="flex-1 px-3 py-2 bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-700 rounded-lg text-xs focus:ring-2 focus:ring-brand-blue/10 focus:border-brand-blue outline-none"
                          />
                        </div>
                      </div>
                    </div>
          ...
          {/* STEP 3: Configurações Regionais + SUPORTE */}
          {step === 3 && (
            <div className="space-y-8 animate-slide-up">
              <div className="flex items-center gap-3 mb-6">
                <div className="w-10 h-10 bg-slate-100 dark:bg-slate-800 rounded-lg flex items-center justify-center text-slate-500">
                ...
                <div>
                  <h3 className="text-lg font-bold text-slate-900 dark:text-white">Configurações</h3>
                  <p className="text-xs text-slate-500">Idioma e canal de suporte.</p>
                </div>
              </div>

              {/* Idioma */}
              <div>
                <label className={labelStyle}>Idioma Padrão</label>
              ...
              <div>
                ...
                <p className="text-[10px] text-slate-400 mb-4 font-medium">
                  Aparecerá como botão flutuante no PWA do aluno
                </p>

                <div className="space-y-4">
                  {/* Opção: Email */}
                  <div
                    onClick={() => updateField('support_type', 'email')}
                    className={cn(
                      "cursor-pointer p-4 rounded-xl border-2 transition-all",
                      formData.support_type === 'email'
                        ? "border-brand-blue bg-blue-50/50 dark:bg-blue-900/10"
                        : "border-slate-200 dark:border-slate-700 hover:border-slate-300"
                    )}
                  >
                    ...
                    {formData.support_type === 'email' && (
                      <input
                        ...
                        onChange={(e) => updateField('support_value', e.target.value)}
                        placeholder="suporte@seuapp.com"
                        className="w-full px-3 py-2 bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-700 rounded-lg text-xs focus:ring-2 focus:ring-brand-blue/10 focus:border-brand-blue outline-none"
                  ...
                  {/* Opção: WhatsApp */}
                  <div
                    onClick={() => updateField('support_type', 'whatsapp')}
                    className={cn(
                      "cursor-pointer p-4 rounded-xl border-2 transition-all",
                      formData.support_type === 'whatsapp'
                        ? "border-brand-blue bg-blue-50/50 dark:bg-blue-900/10"
                        : "border-slate-200 dark:border-slate-700 hover:border-slate-300"
                    )}
                  >
                  ...
                  <button
                    onClick={() => {
                      updateField('support_type', null);
                      updateField('support_value', '');
                    }}
                    className="text-xs text-slate-500 hover:text-slate-700 font-medium underline"
                  >
                    Não configurar suporte agora
                  </button>
                </div>
              </div>
            </div>
                ...
                <div className="space-y-2 text-sm">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium text-slate-900 dark:text-white">
                      app.tribebuild.pro/{formData.slug}
                    </span>
                  </div>
                  <div className="flex justify-between">
                  ...
                  <div className="flex justify-between">
                    <span className="text-slate-500">Banners:</span>
                    <span className="font-medium text-slate-900 dark:text-white">
                      {formData.banners.filter(b => b.image_url).length} configurados
                    </span>
                  </div>
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium text-slate-900 dark:text-white">
                      {formData.support_type
                        ? formData.support_type === 'email' ? 'E-mail' : 'WhatsApp'
                        : 'Não configurado'
                      }
                    </span>
                  </div>
