            ...
            return (
              ...
              >
                ...
                <ul className="space-y-3 mb-8">
                  {plan.features.map((feature, i) => (
                    <li key={i} className="flex items-start gap-3">
                      <div className={`mt-0.5 p-0.5 rounded-full ${plan.highlight ? 'bg-brand-blue/10 dark:bg-brand-blue/20 text-brand-blue' : 'bg-slate-100 dark:bg-slate-800 text-slate-400 dark:text-slate-500'}`}>
                ...
                <div className="flex items-center gap-2 text-slate-400 dark:text-slate-500 text-[10px] font-bold uppercase tracking-widest mb-6 pb-4 border-t border-slate-100 dark:border-slate-800 pt-4 justify-center">
                  <Info className="w-3.5 h-3.5" /><span>{plan.extraInfo}</span>
                </div>
                <button onClick={() => navigate('/register')} className={`block w-full py-4 px-6 rounded-2xl font-black text-sm uppercase tracking-widest transition-all duration-300 text-center ${plan.highlight ? 'bg-brand-blue text-white hover:bg-brand-blue-dark shadow-xl shadow-brand-blue/30 hover:shadow-2xl hover:shadow-brand-blue/40' : 'bg-slate-100 dark:bg-slate-800 text-slate-900 dark:text-white hover:bg-slate-200 dark:hover:bg-slate-700 shadow-sm'} active:scale-95`}>{plan.cta}</button>
              </div>
            );
          })}
        </div>
