...
const DashboardHeader: React.FC<DashboardHeaderProps> = ({ onLogout }) => {
  ...
  return (
    <>
      <header className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border-b border-slate-200/80 dark:border-slate-700/50 sticky top-0 z-50 transition-colors duration-300">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-20">
            ...
            <nav className="hidden lg:flex items-center gap-2">
              {navItems.map((item) => (
                <Link
                  key={item.name}
                  to={item.href}
                  className={`px-4 py-2.5 rounded-xl text-sm font-semibold tracking-tight transition-all ${
                    isActive(item.href)
              ...
              <Link
                ...
                className={`ml-2 px-4 py-2.5 rounded-xl text-sm font-semibold tracking-tight transition-all flex items-center gap-2 ${
                  isActive(bonusItem.href)
                    ...
                    : 'bg-brand-coral/10 text-brand-coral hover:bg-brand-coral/20 border border-brand-coral/20'
                }`}
              >
            ...
            <div className="flex items-center gap-4">
              ...
              <a 
                href="#/academia"
                className="hidden xl:flex items-center gap-2 text-sm font-bold text-slate-400 hover:text-brand-blue transition-colors"
              >
              ...
              <div className="relative hidden md:block">
                ...
                >
                  <div className="w-10 h-10 rounded-xl bg-brand-blue flex items-center justify-center text-white text-sm font-bold shadow-lg shadow-brand-blue/20 group-hover:scale-105 transition-transform">
                    {user.initials}
                  </div>
                ...
                {isUserMenuOpen && (
                  <>
                    ...
                    />
                    <div className="absolute right-0 mt-3 w-64 bg-white dark:bg-slate-900 rounded-3xl shadow-2xl border border-slate-100 dark:border-slate-700 py-3 z-20 animate-slide-up origin-top-right">
                      ...
                      <Link
                        ...
                        className="flex items-center gap-3 px-5 py-3 text-sm font-semibold text-slate-600 dark:text-slate-300 hover:bg-brand-blue/5 hover:text-brand-blue transition-all"
                        onClick={() => setIsUserMenuOpen(false)}
                      >
              ...
              <button
                ...
                className="lg:hidden p-3 text-slate-500 hover:text-slate-900 bg-slate-50 hover:bg-slate-100 rounded-2xl transition-all"
